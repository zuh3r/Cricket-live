function renderTournaments(tournamentsData, startIndex) {
  ...
  document.querySelector(".tournamentsSection").innerHTML = ` 
    ...
    `;
  document
    .querySelector(".tournaments_ul li:nth-child(3)")
    .classList.add("hover");
}
...
function renderMatchSummary(event) {
  ...
  axios
    .get(`/api/matches/match/summary/${matchID}`) //returns data with match status, has all information, from overs, to winner, to man of the match etc etc
    .then((res) => {
